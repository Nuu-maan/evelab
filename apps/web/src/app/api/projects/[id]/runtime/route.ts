...
import { getRuntime, localRuntimeAllowed, startRuntime, stopRuntime } from "@/lib/runtime";
...
export async function GET(_request: Request, { params }: Context) {
  ...
  return Response.json({ runtime: getRuntime(id), allowed: localRuntimeAllowed() });
}
...
export async function POST(request: Request, { params }: Context) {
  ...
  return Response.json({ runtime, allowed: localRuntimeAllowed() });
}
